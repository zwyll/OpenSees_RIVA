"""Run the strict DSS gate on the intermediate biased-flow successor."""
...
def parameters_from_args(
    ...
    defaults = asdict(RIVASandIntermediateBiasFlowParameters())
    allowed = {item.name for item in fields(RIVASandIntermediateBiasFlowParameters)}
    overrides = strict._parameter_overrides(args.parameter, defaults)
    unknown = sorted(set(overrides) - allowed)
    if unknown:
        raise ValueError(
            f"unknown intermediate-flow parameter(s): {', '.join(unknown)}"
        )
    defaults.update(overrides)
    return RIVASandIntermediateBiasFlowParameters(**defaults)

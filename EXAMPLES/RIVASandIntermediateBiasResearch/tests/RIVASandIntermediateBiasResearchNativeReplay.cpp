...
static void append_state_columns(std::vector<std::string> &columns)
{
    ...
    for (const char *name : {
             ...
             "ep_half_last", "field_bias_mean_activity"})
        columns.push_back(std::string("state_") + name);
}
...
static std::vector<std::string> output_header()
{
    ...
    if (columns.size() != 237)
        throw std::runtime_error("internal output schema is not 237 columns");
    return columns;
}
...
static void put_state(Row &row, const riva_ib_state_t &state)
{
    ...
    put(row, "state_field_bias_mean_activity",
        state.field_bias_mean_activity);
}
